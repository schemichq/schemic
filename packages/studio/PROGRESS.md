...
- [x] **Bidirectional cursor sync — true position mapping (Slice 2)** — codegen returns a **source map** (each table/field's `.ts` line ↔ generated line): emit lines tracked via `emitStatements`, source lines via a TS-AST parse, correlated by `table`/`table.field` path. Editor cursor → reveal+mark the exact generated line; preview cursor → reveal+mark the exact source line (accent marker both sides; source flag prevents self-highlight). Precise even with duplicate names across tables. Nested fields not mapped yet (follow-up)
