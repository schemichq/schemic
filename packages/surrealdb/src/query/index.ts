/**
 * `@schemic/surrealdb/query` — the SurrealDB-native, opt-in query builder (REFERENCE prototype to
 * de-risk `@schemic/core/query`). Driver-OWNED surface: the operators + the lowering to SurrealQL live
 * here; the cross-driver machinery (projection inference + the projection decode) is reused from core.
 *
...
type CmpOp = "=" | "!=" | "<" | "<=" | ">" | ">=";
type Expr =
  | { readonly kind: "cmp"; readonly col: string; readonly op: CmpOp; readonly value: unknown }
...
function makeRef(col: string): FieldRef<unknown> {
  const cmp = (op: CmpOp) => (value: unknown): Expr => ({ kind: "cmp", col, op, value });
  return brandRef({
    __col: col,
    eq: cmp("="),
    neq: cmp("!="),
...
function lowerExpr(e: Expr, vars: Record<string, unknown>): string {
  ...
  const joined = e.parts.map((p) => lowerExpr(p, vars)).join(e.kind === "and" ? " AND " : " OR ");
  return `(${joined})`;
}
...
class Select<TD extends TableDef<string, any>, Res> {
  ...
  orderBy(fn: (row: Row<TD>) => FieldRef<unknown>, dir: "asc" | "desc" = "asc"): Select<TD, Res> {
  ...
  return<P extends Record<string, FieldRef<unknown>>>(
    ...
    const proj = Object.entries(shape).map(([as, ref]) => ({ as, col: colOf(ref) }));
  ...
  toSQL(): Lowered {
    ...
    const s = this.state;
    const cols = s.proj
      ? s.proj.map((p) => (p.as === p.col ? escapeIdent(p.col) : `${escapeIdent(p.col)} AS ${escapeIdent(p.as)}`)).join(", ")
      : "*";
    let sql = `SELECT ${cols} FROM ${escapeIdent(this.table.name)}`;
    if (s.where) sql += ` WHERE ${lowerExpr(s.where, vars)}`;
    if (s.order) sql += ` ORDER BY ${escapeIdent(s.order.col)} ${s.order.dir.toUpperCase()}`;
...
export function select<TD extends TableDef<string, any>>(table: TD): Select<TD, App<TD>> {
