...
- [x] **`defineSingleton(name, shape?, { id? })`** (ratified cross-driver) — a DB-ENFORCED one-record table (app config / feature flags): the `id` column is fixed to one literal (default `"default"`), emitted `id text PRIMARY KEY DEFAULT '<id>' CHECK (id = '<id>')` — PK + CHECK make exactly one row possible; `create` fills the id automatically. `App["id"]` is the id's string LITERAL (the compile-time marker read by `SingletonIdOf<T>`, `never` for a normal table). Round-trips with ZERO phantom-diff: the canonical drops the DEFAULT/CHECK to the bare `id text PRIMARY KEY` — the exact line introspect reconstructs (`config.singleton` marker drives emit; `forCanonical` reduces it). The `id` key is reserved (managed here). [ ] id-optional client sugar (`db.get(Config)` / `update` / `delete` without the literal) — follow-up (needs query Phase 1 on the base)
...
- [x] **implicit `id` is AUTO-GENERATED on INSERT** — the implicit id emits with a DB-side `DEFAULT gen_random_uuid()::text` (builtin, no extension), so `db.create(implicitTable).content(...)` (and bare-SQL inserts) fill the id automatically and the created row carries the generated uuid (surreal-parity: the DB fills it, like any `$default`). The default is DROPPED from the `canonical` form (`createTableDdl(..., { forCanonical: true })`), so it's drift-excluded like every DEFAULT — a pre-default snapshot does NOT phantom-diff (introspect skips the implicit id; `isImplicit` ignores the default). NOTE: the default lands only on FRESH `CREATE TABLE` (BREAKING(alpha) — emitted DDL for implicit-id tables changed); existing tables aren't auto-migrated (add it manually or recreate). Want a client-controlled id / a different generator? Declare an explicit `id` column.
