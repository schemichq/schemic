...
- [x] **table composition** (ratified cross-driver surface): `defineTable(name, obj)` also accepts an `s.object()` (uses its `.fields` — the native PgFields with column metadata, NOT the jsonb collapse); `PgTableDef.extend(record | s.object())` composes more columns in (typed, cast-free — the base-column mixin primitive). `s.object().fields` is the public native field map (inverse of `.shape`), retained through `.extend/.pick/.omit/…`
- [x] **table presets** (ratified cross-driver primitive) — `defineTable.preset({ columns?, indexes? })` returns a reusable, configurable bundle (usually a fn); `PgTableDef.use(preset)` merges it: `columns` TYPED-merged (they appear in row/`.create`/`.update`; a name conflict — vs the table or another preset — is a COMPILE error AT THE CALL + a runtime throw), `indexes` append (reuses `.extend`'s cast-free merge). `.use` is SINGLE-ARG + CHAINED (`.use(a).use(b)`, ratified cross-driver with surreal): each step re-derives the field type so it stays fully typed at any depth AND catches preset-vs-preset clashes — variadic N-way typed-merge degrades/crashes tsc, so chaining is the contract. The four key NAMES (`columns/permissions/events/indexes`) are shared cross-driver; CONTENTS dialect-native. NOTE: a dynamic column name needs a `<K extends string>` generic to stay a literal key (a widened `string` becomes an index signature). [ ] `permissions` → RLS **policies** + `events` → **triggers** (append; separate-object kinds — need explode wiring) are the follow-up; SECURITY: a pg preset `permissions` policy is a pg RLS policy combined by pg (PERMISSIVE = OR, so it CAN widen access — unlike surreal presets which only narrow; use the restrictive flag to scope)
- [x] **derived input schemas** `PgTableDef.create` / `.update` (Standard-Schema-composable z.objects, `.or/.extend/.partial/.refine`). Precise TYPES via the `Flags` channel: `$default`/`$identity` brand `"create"` (optional on create), `$identity`/`$generated`/`$primaryKey` brand `"readonly"` (excluded from update), `$generated` brands `"generated"` (excluded from create). `.create` drops generated + makes default/identity/`id` optional; `.update` drops `id`/readonly + partials the rest. App types `CreateInput<T>`/`UpdateInput<T>`. (Mirrors surreal's `CreateShape<S>`; the `string extends FlagsOf` guard keeps shape-agnostic `PgTableDef` refs intact.)
...
- [x] **raw `pgSql` fragment as a typed OPERAND** — every comparison + `in`/`notIn` takes `Operand<T> = T | BoundPgQuery`, so `u.age.gte(pgSql\`24\`)` / `u.at.lt(pgSql\`now() - interval '1 day'\`)` / `u.age.in([pgSql\`24\`, 30])` compose raw SQL into the typed builder; the fragment SPLICES (parenthesized) with its own `$n` renumbered + params merged, a plain value still binds. The general escape hatch (raw SQL where the typed op set doesn't reach) without dropping to `db.query`
- [x] **raw `pgSql` PREDICATE in `where`** — `where(u => pgSql\`${u.age} > 24\`)`: a whole raw boolean fragment is a predicate leaf (spliced, parenthesized, binds merged). Column refs carry the `__pgRaw` splice protocol (the same one `identifier()`/`raw()` use), so interpolating `${u.age}` emits its quoted identifier instead of binding it — mixing refs and values in one template (`pgSql\`${u.age} > ${24}\`` → `("age" > $1)`)
- [x] **`Expr.and/.or/.not` chainable combinators** (+ free `and`/`or`/`not`) — every one accepts a `Predicate = Expr | BoundPgQuery`, so typed exprs and raw fragments compose freely: `u.age.gte(18).and(pgSql\`${u.name} <> 'x'\`).not()`
- [x] **Phase-1 WHERE ops (cross-driver contract)** — `in`/`notIn` (`IN (…)`/`NOT IN`; empty set → `FALSE`/`TRUE`), `isNone`/`isNotNone` (`IS [NOT] NULL`) on EVERY column; **type-narrowed** ops: `startsWith`/`endsWith`/`includes` on STRING columns only (`starts_with(col,$1)` / `right(col, char_length($1)) = $1` / `strpos(col,$1) > 0`), `contains`/`containsAny`/`containsAll` on ARRAY columns only (`$1 = ANY(col)` / `col && $1` / `col @> $1`). Narrowing = `FieldRefOps<T>` base + conditional `StringRefOps`/`ArrayRefOps<E>` intersections + `IsAny` guard (mirrors surreal exactly; `ts-expect-error`-locked). **`includes`** = the RATIFIED cross-driver string-substring op (case-SENSITIVE, NULL column never matches, matches `z.string().includes()`; `contains*` stays array-membership only — pg lowers to `strpos`, surreal `CONTAINS`, sqlite `instr`). Requires `s.array(elem)` to carry its element type into `App`.
...
- [x] **implicit `id` is CARRIED on returned rows** (cross-driver convention) — a table's implicit `id text PRIMARY KEY` is added at emit time, not a declared field, so it's outside `object`/`App`/the row refs. But `select` and write-`RETURNING` now FETCH it and splice it onto the decoded row, typed `App<TD> & { id: string }` (`RowOf<TD>`), so `const c = await db.create(T)…; db.update(T, c.id)` chains (surreal injects `id: RecordId`). Still NOT in the row refs — to `.where(u => u.id…)` FILTER on it, declare an explicit `id` column (`id: s.uuid()`, `s.text().$primaryKey()`, …).
- [ ] **implicit `id` is not auto-generated on INSERT** — the implicit `id text PRIMARY KEY` has no DEFAULT and `create` excludes it, so `db.create(implicitTable).content(...)` fails the not-null PK (an implicit-id table is read/update/delete-only via the ORM; supply an id via raw SQL, or declare `id: s.uuid().$default(sqlExpr("gen_random_uuid()"))`). FOLLOW-UP flagged to core-dev: give the implicit id an auto-gen default (surreal-parity) so implicit-id tables are insertable.
- [x] **single-record WRITES (P2)** — `create(T).content(data)` (INSERT, validated vs `T.create`), `update(T, id).merge(patch)` / `.set(patch)` (partial UPDATE vs `T.update`) / `.content(row)` (replace vs `T.create`), `remove(T, id)` (DELETE; standalone export named `remove` — `delete` reserved; client method `db.delete`). All by SINGLE-COLUMN pk (`config.primaryKey` else implicit `id`; composite throws → use `db.query`). `.return("after"` default full row `| "none"` | projection callback `r => ({…})`); RETURNING lowered + decoded (full row via `.decode`, projection via core `decodeProjection`). Validation is codec-channel FAIL-FAST — `z.encode(T.create|update, …)` throws the ZodError at the `.content`/`.merge`/`.set` CALL SITE (not at `await`) and doubles as app→wire lowering. Thenable + `.run(conn?)`; pre-bound on the client. Mirrors surreal's ratified split-builder surface. NOTE pg flat columns → `.merge` and `.set` coincide (partial SET); surreal-native `before`/`diff` return modes are omitted (not pg-native)
- [n/a] joins / CTEs / sub-selects / aggregates — later phases (Phase-0 is single-table SELECT)

### ORM client — `@schemic/postgres/client` (P1: reads · P2: writes)
...
- [x] typed `config.connect(name, args?)` — `postgresConnection(...)` embeds a lazy `client` opener + a dialect `label` (url | `pglite(memory)`, for bulk reporting), typed as `ConnectionEntry<PgClient, Args>`. Args are the resolver's typed SECOND parameter (connections v2 — no args schema to plumb; annotate the param and `Args` is inferred), so `defineConfig(...).connect(name, args)` autocompletes names + types args + infers the `PgClient` per entry. `client.connect(name, args?)` passes the same args through for a parameterized/bulk selection
- [x] **typed cross-connections** — `postgresConnection` is a valid **chained-builder driver marker**: assignable to core's `ChainableDriverFactory<PostgresConnectionConfig, PgClient>` (two overloads config/resolver + a UNIFIED `ConnectionInput` overload so the whole union is one signature), so `defineConfig().connection("main", postgresConnection, …).connection("replica", postgresConnection, (ctx, args) => …)` types through it — `ctx.connections.<prior>` is a THENABLE sibling **`PgClient`** (`const main = await ctx.connections.main; main.select(…)`, opened via the embedded client opener), accumulated in declaration order (a forward reference is a compile error). Factory now mirrors `surrealConnection`'s two-overload shape (a fleet `key` is optional, per the `label` hook — not forced by the array form)
...
- [x] **writes (P2)** — `db.create(T).content(data)` / `db.update(T, id).merge|content|set(...)` / `db.delete(T, id)`, all pre-bound + thenable (`await db.create(User).content({…})`). Split builder + codec-channel fail-fast validation; see the `./query` writes line above for the full surface
- [n/a] `db.call` (function invoke) + a scoped `db.session()`/transaction — later (pg session = a `BEGIN`/`COMMIT` tx scope; degenerate for PGlite reads, deferred)
